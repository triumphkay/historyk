...
### data-pipeline/ 주요 흐름
- `scripts/make_database.py`: 파이프라인 전체를 순차 실행하여 `sessions` → `events` → `keywords` 테이블을 재생성하고 `key-timeline`/`key-age`/`key-prehistoric` 데이터를 갱신합니다.
...
- `scripts/copy-json.js` (`app/` 안): 파이프라인에서 만든 `keyword-types.json`을 앱 자산 디렉터리에 복사합니다.
...
- 주요 Context:
  ...
  - `NewWordEraQuizContext`: 시대 퀴즈 상태와 시대/왕/정부 선택 로직
