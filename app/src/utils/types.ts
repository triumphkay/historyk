export const normalizeTypeString = (typeString = ""): string =>
  typeString.toString().trim().toLowerCase().replace(/\s+/g, "");
...
export const pickDisplayType = (types: string[] = []): string | null => {
  ...
  const typeDetails =
    (typeMetadata as { "type-set": Array<{ title: string; age?: boolean }> })[
      "type-set"
    ] || [];
  ...
  // Filter out "시기" type
  const nonSigiTypes = types.filter(
    (type) => typeof type === "string" && !isSigi(type)
  );
